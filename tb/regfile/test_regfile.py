...
async def random_write_read_test(dut):
    ...
    await RisingEdge(dut.clk)
    dut.rst_n.value = 1 # release rst_n to prevent from resetting
    await RisingEdge(dut.clk)

    # theoretical state of the regs
    theorical_regs = [0 for _ in range(32)]

    for _ in range(1000):
        # Generate random register addresses
    ...
    # try to write 0 and check if it's still 0
    await Timer(1, unit="ns")
    dut.address3.value = 0
    dut.write_enable.value = 1
